     ...
     )
{
     ...
     initl
         (
           ...
           &ainv, &ao, &sat->con41, &con42, &cosio, &cosio2, &eccsq, &omeosq,
     ...
     if ((omeosq >= 0.0 ) || ( sat->no >= 0.0))
       {
         ...
         cc2   = coef1 * sat->no * (ao * (1.0 + 1.5 * etasq + eeta *
                        (4.0 + etasq)) + 0.375 * j2 * tsi / psisq * sat->con41 *
                        (8.0 + 3.0 * etasq * (8.0 + etasq)));
         sat->cc1   = sat->bstar * cc2;
         cc3   = 0.0;
         if (sat->ecco > 1.0e-4)
             cc3 = -2.0 * coef * tsi * j3oj2 * sat->no * sinio / sat->ecco;
         ...
         temp1  = 1.5 * j2 * pinvsq * sat->no;
         temp2  = 0.5 * temp1 * j2 * pinvsq;
         temp3  = -0.46875 * j4 * pinvsq * pinvsq * sat->no;
         sat->mdot     = sat->no + 0.5 * temp1 * rteosq * sat->con41 + 0.0625 *
         ...
         if ((2*PI / sat->no) >= 225.0)
           {
             ...
             dscom
                 (
                   ...
                   sat->no, &snodm, &cnodm,  &sinim, &cosim, &sinomm,     &cosomm,
             ...
             dsinit
                 (
                   ...
                   sat->gsto, sat->mo, sat->mdot, sat->no, sat->nodeo,
